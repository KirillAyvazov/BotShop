...
def test_no_connection(seller_pool, user_id, url_no_valid, data_base):
    """
    ...
    из пула, данные на сервере не должны быть изменены.
        ...
        - проверяем что значения атрибутов объекта user registered_on_server (зарегистрирован на сервере) = False и
            is_changed(изменен) = False
    """
    setattr(seller_pool, "_user_url", url_no_valid)
...
def test_recovery_connection(seller_pool, user_id, url_no_valid, shopper_url, data_base):
    """
    ...
    setattr(seller_pool, "_user_url", shopper_url)
...
def test_no_connection_and_change_user_part_1(seller_pool, user_id, url_no_valid, shopper_url, data_base):
    """
        Тест работы пула покупателей в условиях неудавшегося запроса к API на этапе получения данных пользователя.
...
def test_no_connection_and_change_user_part_2(seller_pool, user_id, url_no_valid, shopper_url, data_base):
    """
