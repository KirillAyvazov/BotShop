...
def execute_in_new_thread(
    _func: Optional[Callable] = None, *, daemon: bool = False
) -> Callable:
    """
            ...
            key = "".join(
                [
                    ...
                    *[
                        str(i_arg)
                        for i_arg in args
                        if isinstance(i_arg, Union[str, int, List, Tuple, Dict, float])
                    ],
                    *kwargs.keys(),
                ]
            )
    ...
    def __data_control(self) -> None:
        ...
        while True:
            ...
            for i_key, i_data in list(self.__memory.items()):
                if datetime.now(moscow_tz) - i_data.saving_time > timedelta(
                    seconds=43200
                ):
                    with Semaphore():
            ...
            dev_log.debug(
                f"Размер кэша {self.__name__} до/после очистки: {start_size}/{new_size}"
            )
...
def timer(func: Callable) -> Optional[Any]:
    ...
    def wrapped(*args, **kwargs):
        ...
        print(
            f"Время выполнения функции {func.__name__} - {round(time.time()-time_start, 3)}"
        )
        return result
...
class DataTunnel:
    ...
    def add_methods(self, *methods_name) -> Callable:
        ...
        def decorator(cls) -> Callable:
            ...
            def wrapped(*args, **kwargs) -> object:
                ...
                methods = {
                    ".".join([cls.__name__, i_name_methods]): getattr(
                        cls_obj, i_name_methods, None
                    )
    ...
    def add_func(self, func: Callable) -> Callable:
        ...
        if func.__name__ in self.__func_dict:
            raise ValueError(
                "Функция с именем {} уже зарегистрирована в тоннеле данных".format(
                    func.__name__
                )
            )
    ...
    def perform(self, func_name: str, *args, **kwargs) -> Optional[Any]:
        ...
        raise ValueError(
            "Функция с именем {} НЕ зарегистрирована в тоннеле данных".format(func_name)
        )
