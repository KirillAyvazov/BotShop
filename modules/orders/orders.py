...
class Order:
    ...
    def __get_product_obj(self) -> None:
        ...
        for i_dict in self.products_data:
            ...
            product: Product = data_tunnel.perform("CategoryPool.get_product", product_id)
    ...
    def set_product_count(self, product_id: Union[Product, str], count: int) -> None:
        ...
        list_dict = list(filter(lambda i_dict: key in i_dict.values(), self.products_data))

        if len(list_dict) == 1:
            list_dict[0]["count"] = count
        elif len(list_dict) > 1:
            dev_log.warning("При изменении количества товаров в заказе, было найдено несколько товаров с одинаковым id")
        elif len(list_dict) < 1:
            dev_log.warning("При изменении количества товаров в заказе, товар по указанному id не был найден")
    ...
    def _get_hash_sum(self) -> int:
        ...
        order_products = ''.join([''.join([str(i_product),
                                           str(self.get_product_count(i_product.productsId))])
    ...
    def __repr__(self) -> str:
        ...
        for index, i_product in enumerate(self.products):
            ...
            text.append('{num}. {category_name}: {prod_name} - {count} шт. = {sum} рублей'.format(num=index+1,
                        category_name=i_product.category, prod_name=i_product.name, count=str(product_count),
...
class Basket(Order):
    ...
    def add_product(self, product: Product, count: int) -> None:
        ...
        if len(product_in_basket) > 0:
            ...
            self.products_data.append({"productsId": product.productsId, "count": count})

        self.__update_total_cost()

    def __update_total_cost(self):
        """Метод пересчитывает общую стоимость всех товаров в корзине"""
        self.totalCost = sum([self.get_product_count(i_product) * i_product.price for i_product in self.products])
    ...
    def __repr__(self) -> str:
        ...
        if len(self.products) > 0:
            ...
            for index, i_product in enumerate(self.products):
                ...
                text.append(f'{index + 1}. {i_product.category}: {i_product.name} x {i_product_count} ='
    ...
    def get_list_product_name(self) -> List[str]:
        ...
        for index, i_product in enumerate(self.products):
            i_product_count = self.get_product_count(i_product)
            list_product_name.append(f'{index+1}. {i_product.category}: {i_product.name} - {i_product_count} шт.')
...
class OrderSchema(Schema):
    ...
    products_data = fields.Nested(ProductDataSchema, many=True, required=True, data_key="products")
